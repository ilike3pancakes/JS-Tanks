function debugTank(tank, arena) {
    ...
    // CHEATING MY ASS OFF
    let cheat = null;
    tanks.forEach((other, i) => {
        if (tank.index === other.index) {
            cheat = tanks[i];
        }
    });
    cheat.energy = 999999999;
    tank.color = getRandomColorHex();
    ...
    if ((keyDown["ArrowUp"] || keyDown["w"]) && (keyDown["ArrowRight"] || keyDown["d"])) {
        desiredAngle = 315; // Up-Right
        desiredSpeed = 1;
    }
    if ((keyDown["ArrowUp"] || keyDown["w"]) && (keyDown["ArrowLeft"] || keyDown["a"])) {
        desiredAngle = 225; // Up-Left
        desiredSpeed = 1;
    }
    if ((keyDown["ArrowDown"] || keyDown["s"]) && (keyDown["ArrowRight"] || keyDown["d"])) {
        desiredAngle = 45;  // Down-Right
        desiredSpeed = 1;
    }
    if ((keyDown["ArrowDown"] || keyDown["s"]) && (keyDown["ArrowLeft"] || keyDown["a"])) {
